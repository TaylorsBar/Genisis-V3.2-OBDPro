...
let content = fs.readFileSync('services/OpticalFlowProcessor.ts', 'utf8');
...
const oldClassRegex = /export class OpticalFlowProcessor \{[\s\S]*\}\s*$/;
const newContent = content.replace(oldClassRegex, opticalClass);
fs.writeFileSync('services/OpticalFlowProcessor.ts', newContent);
console.log("Optical flow updated!");
