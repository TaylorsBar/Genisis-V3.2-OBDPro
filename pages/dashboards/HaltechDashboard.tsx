...
const DataTile = ({ label, value, unit, color = "text-white", border = false, warning = false }: { label: string, value: string, unit: string, color?: string, border?: boolean, warning?: boolean }) => (
    <div className={`flex flex-col justify-center p-2 md:p-4 bg-surface-panel backdrop-blur-sm border border-white/5 ${border ? 'border-l-4 !border-l-[var(--theme-color)]' : ''} ${warning ? 'bg-red-900/20 border-red-500 animate-pulse' : ''} rounded-md min-w-[80px] md:min-w-[120px] shadow-md h-full`}>
